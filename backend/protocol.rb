  ...
  module Protocol
    ...
    class Simple
      ...
      def preprocess_message(message)
        Plugin.href_maker(Plugin.sanitize(message))
      end
        ...
        case event
          ...
          when :message
            ...
            case message[:type]
              ...
              when :send_broadcast
                message = preprocess_message(data[:params][:message])
                server.broadcast wsh, message

              when :send_private
                message = preprocess_message(data[:params][:message])
                server.private wsh, data[:params][:recipient], message
            end
